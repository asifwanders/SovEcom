 ...
 * conflict-update), so `exec` always reports that one row.
 */
...
export class FakeTables implements TablesClient {
  ...
  exec(
    ...
    // recordView: INSERT ... ON CONFLICT (viewer_key, product_id) DO UPDATE SET viewed_at = now() RETURNING ...
    if (s.startsWith('INSERT INTO') && s.includes('ON CONFLICT')) {
      ...
      if (existing) {
        // Conflict → DO UPDATE SET viewed_at = now() (a bump). The id stays the original.
        existing.viewed_at = this.nextTs();
