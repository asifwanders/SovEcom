...
export class FakeTables implements TablesClient {
  ...
  query<T = Record<string, unknown>>(
    ...
    // ── Customer counts ──────────────────────────────────────────────────────
    if (s.startsWith('SELECT COUNT(*)') && s.includes('mod_wishlist_items')) {
    ...
    // ── Guest counts ─────────────────────────────────────────────────────────
    if (s.startsWith('SELECT COUNT(*)') && s.includes('mod_wishlist_guest_items')) {
      const guestId = String(params[0]);
      const count = this.guestItems.filter((r) => r.guest_id === guestId).length;
      return Promise.resolve({ rows: [{ count } as unknown as T], rowCount: 1 });
    }
    // ── Customer has (LIMIT 1) ────────────────────────────────────────────────
    if (
      s.includes('FROM mod_wishlist_items') &&
      s.includes('WHERE customer_id = $1 AND product_variant_id = $2 LIMIT 1')
    ) {
    ...
    // ── Guest has (LIMIT 1) ───────────────────────────────────────────────────
    if (
      s.includes('FROM mod_wishlist_guest_items') &&
      s.includes('WHERE guest_id = $1 AND product_variant_id = $2 LIMIT 1')
    ) {
      const [gid, vid] = [String(params[0]), String(params[1])];
      const found = this.guestItems.filter(
        (r) => r.guest_id === gid && r.product_variant_id === vid,
      );
      return Promise.resolve({ rows: found as unknown as T[], rowCount: found.length });
    }
    // ── Customer list (ORDER BY) ──────────────────────────────────────────────
    if (s.includes('FROM mod_wishlist_items') && s.includes('WHERE customer_id = $1 ORDER BY')) {
    ...
    // ── Guest list (ORDER BY) ─────────────────────────────────────────────────
    if (s.includes('FROM mod_wishlist_guest_items') && s.includes('WHERE guest_id = $1 ORDER BY')) {
      const gid = String(params[0]);
      const rows = this.guestItems
        .filter((r) => r.guest_id === gid)
        .sort((a, b) => (a.created_at < b.created_at ? 1 : -1));
      return Promise.resolve({ rows: rows as unknown as T[], rowCount: rows.length });
    }
    // ── Customer watching (IN list, for digest) ────────────────────────────────
    ...
    // ── Customer select-after-upsert (for add return value) ────────────────────
    if (s.includes('FROM mod_wishlist_items') && s.includes('LIMIT 1')) {
      const [cid, vid] = [String(params[0]), String(params[1])];
      const found = this.items.filter((r) => r.customer_id === cid && r.product_variant_id === vid);
      return Promise.resolve({ rows: found as unknown as T[], rowCount: found.length });
    }
    // ── Guest select-after-upsert (for guestAdd return value) ─────────────────
    if (s.includes('FROM mod_wishlist_guest_items') && s.includes('LIMIT 1')) {
      const [gid, vid] = [String(params[0]), String(params[1])];
      const found = this.guestItems.filter(
        (r) => r.guest_id === gid && r.product_variant_id === vid,
      );
  ...
  exec(
    ...
    // ── Guest insert ──────────────────────────────────────────────────────────
    if (s.startsWith('INSERT INTO mod_wishlist_guest_items')) {
      const [, gid, vid] = [String(params[0]), String(params[1]), String(params[2])];
      const exists = this.guestItems.some(
        (r) => r.guest_id === gid && r.product_variant_id === vid,
      );
      if (exists) return Promise.resolve({ rows: [], rowCount: 0 }); // ON CONFLICT DO NOTHING
    ...
    // ── Guest delete ──────────────────────────────────────────────────────────
    if (s.startsWith('DELETE FROM mod_wishlist_guest_items')) {
      const [gid, vid] = [String(params[0]), String(params[1])];
      const removed = this.guestItems.filter(
        (r) => r.guest_id === gid && r.product_variant_id === vid,
      );
      this.guestItems = this.guestItems.filter(
        (r) => !(r.guest_id === gid && r.product_variant_id === vid),
      );
      const rows = removed.map((r) => ({ id: r.id }));
      return Promise.resolve({ rows, rowCount: rows.length });
    ...
    // ── Digest log delete (unmarkDigested) ────────────────────────────────────
 ...
 */
export type FakeEmailOutcome = 'queued' | 'suppressed' | { throwCode: string };
  ...
  sendToCustomer(message: ModuleCustomerEmailMessage): Promise<{ queued: boolean }> {
    ...
    if (typeof outcome === 'object') {
      return Promise.reject(
        Object.assign(new Error('rpc: send failed'), { code: outcome.throwCode }),
      );
    }
